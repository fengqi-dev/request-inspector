...
Request Inspector helps developers inspect outgoing HTTP requests while running and debugging code in VS Code. It starts the system `mitmproxy` command in a managed VS Code Terminal and automatically injects proxy environment variables into VS Code debug sessions while the proxy is active.

Once `mitmproxy` is installed, the goal is zero configuration for day-to-day development: start the proxy, run your code, and check whether the requests your program sends are correct.

## Features

- System `mitmproxy` runtime: uses the `mitmproxy` command available on your `PATH`.
...
Install `mitmproxy` on your system before starting the proxy. Request Inspector runs the `mitmproxy` command from your `PATH` and does not download or install a runtime automatically.
